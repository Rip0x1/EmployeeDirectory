...
### Запуск приложежения №2
- Иметь запущенный сервер ```PostgreSQL```
- Запустить обязательно главное приложение ```EmployeeDirectory.exe```
- По желанию установить на ANDROID мобильное приложение ```EmployeeDirectory.apk``` и запустить его
- Порты 5000 и 7777 должны быть свободны
